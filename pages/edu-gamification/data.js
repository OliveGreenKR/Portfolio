...
window.EDU_GAMIFICATION_DATA = {
  meta: {
    ...
    oneLine:
      ...
      '첫 웹 풀스택을 **AI에게 「왜」를 캐물어 이해로** 빠르게 흡수 — 무거운 건 관리형에 맡기고 **통합·안전 설계**에 집중.',
    period: '2026.05 – 2026.07 (납품 완료)',
    weeks: '웹 ≈6주 · 계약 마감 8일 전 납품',
    team: '외주 · 프리랜서',
    role: '웹 주개발 (+ 게임 초기 구조 설계)',
    platform: 'Unity 6.1 · React · AWS Serverless · AI 협업',
...
if origin == "api":
  ...
  return acc;
    ...
    /* ─── 5.9 AI '이해' 학습 · 통합·안전 집중 ───────────── */
    {
      no: '5.9',
      kind: 'PROCESS',
      title: "AI '이해' 학습 — 관리형에 위임하고, 통합·안전에 집중",
      lede: '첫 웹 풀스택 · 첫 클라우드 보안 · 외부 지정 스택. 무거운 것은 AWS 관리형에 위임하고, 본인 역량은 "모듈을 어떻게 엮고 어떻게 안전하게 제공하나"에 집중. 그 과정을 AI에 "왜"를 캐물어 이해로 흡수 → 일 단위로 바뀌는 요구에 대응.',
      problem:
        '첫 웹 풀스택이자 첫 클라우드 보안, 스택(React · Lambda · DynamoDB · Terraform · Electron)은 외부가 지정했다. ' +
        '전부 직접 구현할 시간도 근거도 없고 요구는 일 단위로 바뀐다. AWS 서비스는 남이 만든 복잡한 블록이라 "그냥 붙이면" 왜 그렇게 되는지 모른 채 쌓인다.',
      decision:
        '**두 겹의 선택.** ' +
        '(1) **추상화 레벨** — 관리 부담이 큰 것(DB 운영 · 쿼리 확장 · 동시성 · 다중성)은 **AWS 관리형 서비스에 위임**하고, ' +
        '본인 역량은 **① 모듈 단위 시스템을 어떻게 엮나(통합) · ② 어떻게 안전하게 제공하나(보안 · 감사)** = 서비스 층에 집중한다. "무엇을 직접 안 짤지"를 먼저 판단. ' +
        '(2) **학습 방식** — AI를 코드 생성기가 아니라 **"왜?"를 캐묻는 상대**로 쓴다. 모든 설계 선택의 근거를 되묻고(왜 이 서비스 · 왜 이 구조로 엮나), ' +
        '웹 공격이 **왜 발생하고 어떤 원리로 어떻게 조치되는지**를 납득할 때까지 물은 뒤 채택한다 — 생성물 수용이 아니라 **이해로 검증**. ' +
        '이해가 선 상태라 스택이 낯설어도 아키텍처는 스스로 결정하고, 요구가 바뀌어도 원리에서 다시 판단한다.',
      results: [
        '"직접 구현 vs 관리형 위임"을 나눠, 역량을 통합 · 안전 층에 집중 (3,000명 규모 서비스)',
        '이해 기반이라 낯선 스택에서도 핵심 아키텍처를 스스로 결정 (단일 인가 게이트 · 체크포인트 큐 · 스탬프 무임승차 차단)',
        '일 단위 요구 변화 · 대규모 rebuild · 재구조화 3회를 "이해 심화"로 흡수 — 주 66→93→97 커밋',
        '첫 웹 · 첫 클라우드 보안에도 계약 마감 8일 전 납품, 남은 한계는 인수 문서에 정직하게 기재',
      ],
      stack: ['AWS 관리형 위임 (DynamoDB · Lambda · CloudFront)', '본인 집중 = 통합 · 안전 · 서비스 층', "AI '왜?' 질문 → 이해 검증 후 채택", '스펙 주도 개발 (요구→설계→구현→리뷰)', '외부 지정 스택 빠른 흡수'],
      mermaid: `graph TB
    subgraph MANAGED["AWS 관리형에 위임 — 무거운 것"]
        DBM["DB 운영 · 쿼리 확장"]
        SCALE["동시성 · 다중성 · CDN"]
    end
    subgraph MINE["본인 집중 — 판단 · 설계"]
        INT["모듈을 어떻게 엮나<br/>통합"]
        SEC["어떻게 안전하게<br/>보안 · 감사"]
    end
    LOOP["AI '왜?' 질문<br/>이해 검증 후 채택"]
  ...
  metrics: {
    ...
    rows: [
      ...
      ['첫 웹 풀스택 · 일 단위 변경',   "AI '이해' 학습 + 관리형 위임 · 통합·안전 집중", '스스로 아키텍처 결정 · 마감 8일 전 납품'],
    ],
  },
};
