...
window.MOTELET_DATA = {
  ...
  hook:
    '밸런싱 기준은 "성장이 잘 느껴지는가"였다. ' +
    '주관적인 성장 체감을 **계산 가능한 판단 기준**으로 바꿨다.',
  ...
  built: [
    ...
    { kind: 'RUNTIME · §03', href: '#runtime', title: '전투 런타임 구조',
  ...
  model: {
    ...
    minTable: {
      ...
      rows: [
        ...
        ['처치율의 `min`', '스폰이 병목인 구간의 **공격력 증가분**', '**DPS를 기준 지표로 안 쓴 이유**'],
      ],
    },
    ...
    code: {
      ...
      result: '각 레벨에서 공격력 항과 스폰 항 중 어느 쪽이 병목이었는지 집계한다.',
    },
  },
  ...
  sim: {
    ...
    boundary: {
      ...
      outputs: [
        ...
        { label: '노드 간 순위 · 기울기', note: '같은 가정 안에서 비교한다', taken: true },
      ],
      caption:
        '게임 자산에서 읽는 값과 내가 정한 가정값을 **자산 단위로 분리했다.** ' +
        '가정값은 밸런스 시뮬 전용 자산 하나에 모았고, 필드 주석에도 **"가정값"**, **"과소평가 보정"**이라고 명시했다. ' +
        '실제와 어긋날 때는 식이 아니라 이 자산의 노브를 조정한다.',
    },
    ...
    splitNote:
      ...
      '필드 주석에도 **"가정값"**, **"과소평가 보정"**이라고 명시했다. ' +
      '실제와 어긋날 때는 **식이 아니라 이 노브를 조정한다.**',

    points: [
      ['절대값은 가정에 딸려 있다',
        '기대 골드의 절대량은 위 오른쪽 열에 통째로 의존한다. ' +
        '따라서 **같은 가정 아래 노드 간 순위**만 비교 기준으로 사용한다.'],
      ['그래서 읽는 대상을 바꿨다',
        '값이 아니라 **순위와 기울기**. "이 노드가 저 노드보다 몇 배 세다" 가 판단 단위다.'],
      ['비교 기준은 완전체가 아니라 진행형',
        '모든 노드를 켠 빌드는 데미지 과포화로 **거의 모든 값이 0에 수렴**한다. ' +
    ...
    policy: {
      title: '곡선은 §01의 루프를 그대로 돌려서 나온다',
      body:
        '구매만 사람이 아닌 정책이 수행한다. 정책은 **투자 대비 증가분 최대**와 **최저가**, 두 가지다. ' +
        '이 시뮬레이션의 목적은 플레이를 재현하는 것이 아니라 성장 곡선의 흐름을 비교하는 데 있다. ' +
        '구매 전후의 증가분을 기록해 값이 급변한 판과 그때 구매한 노드를 함께 확인했고, 이를 초기 과성장 점검에 사용했다.',
    },
    ...
    shot: {
      ...
      callouts: [
        ...
        { n: 5, x: 89, y: 19, label: '소스/가정 경계', body: '게임 자산 필드와 별도 표시한 가정 자산 한 줄을 구분한다' },
  ...
  runtime: {
    gist:
      '인크레멘탈 게임에서는 한 화면에 적·효과·UI가 계속 나타나고 사라진다. ' +
      '이 반복을 감당하도록 전투씬을 **주입 계약, 선택적 적재, 단발 질의, 메시 배치** 구조로 설계했다.',
    ...
    pipeline: {
      ...
      caption:
        ...
        '외부가 넘기는 값은 **절대값만**이다 — 배율·Rate를 주입에서 없앴다. ' +
        '그래서 같은 씬을 Mock Provider로 단독 실행할 수 있고, 밸런싱 도구가 만든 값도 같은 통로로 들어온다.',
      rules: [
        ['주입은 절대값만', '배율·Rate를 주면 같은 수치가 두 곳에서 곱해진다'],
    ...
    contract: {
      ...
      intro: '월드에 들어오는 조건은 이 인터페이스를 구현하는 것 하나다. 지면을 위해 null 가드 두 줄을 생략했다.',
      code:
        ...
        '}',
      result:
        '질의 결과는 `IGeoBody` 공통 타입으로 반환되며, 호출자는 구체적인 바디 종류와 분리된다.',
    },
    ...
    batch: {
      ...
      afterItems: ['등록된 목록을 LateUpdate에 순회', '정점 버퍼 하나에 이어붙임', '메시 1개 업로드'],
      keyLabel: '배치 키 = 머티리얼 · 소팅 레이어 · 순서',
      result: '요청자가 늘어도 제출은 배치 키 조합 수만큼만 는다',
      caption:
        '데미지 텍스트는 **GameObject가 아예 없다** — 구조체 목록만 들고, 뜨고 사라지는 연출을 ' +
    ...
    why: {
      title: 'Unity 2D Physics를 안 쓴 이유',
    ...
    whyNote:
      '서로 다른 실행 주기를 맞추는 책임이 게임 로직으로 번진다.',
