...
window.MOTELET_DATA = {
  ...
  sim: {
    ...
    shot: {
      ...
      caption:
        '**아래쪽 구간 분석**이 실제로 본 화면 — 어느 판에서 무엇을 사서 얼마나 뛰었는지. ' +
        '가운데는 성장률(파랑 = 시뮬 / 주황 = 목표), 위는 다음 절의 탐색 패널이다.',
      note: '2026-06 편집 상태 캡처.',
      missing: '⚠️ **히트맵이 칠해진 스킬트리 화면이 이 자리에 있어야 한다** — 이 절의 산출물인데 캡처가 없다.',
    },
    bridge:
      '여기까지가 **실제로 굴린 방식**이다. 하나 더 만들었는데 결정에는 쓰지 못했다.',
  },

  /* ── 04 만들고 쓰지 못한 것 ──────────────────────────── */
  search: {
    gist:
      '목표 곡선을 주면 노드 값을 자동으로 맞추는 탐색을 붙였다. ' +
      '**만들었지만 실제 밸런싱 결정에는 쓰지 못했다.**',
    points: [
      ['왜 못 썼나',
        '탐색이 내놓는 건 **가정값 위에서 최적인 벡터**다. 절대값을 안 믿기로 한 이상 그걸 자산에 쓸 근거가 없다.'],
    ],
    fold: {
      title: '만든 내용 — (1+λ)-ES 와 목적함수',
      headers: ['항목', '무엇'],
      rows: [
        ['왜 ES', '목적함수가 시뮬 전체를 통과해야 나와 **미분이 없다** — 경사법 불가'],
        ['방법', '벡터 하나에서 후보 λ개 가우시안 변이 → 최선 1개만 남김'],
        ['오차 척도', '절대 골드 ❌ / **판마다의 로그 증가율** ✅ — 총액이 자릿수로 커지는 게임'],
        ['곡선항', '로그 증가율과 목표 곡선의 오차'],
        ['비율항', '완전체 빌드에서 공격원별 골드 기여 비중을 목표 배분에'],
        ['범위항 · 한계효용항', '스탯 범위 이탈 / 사도 증가분이 죽은 노드에 벌점'],
        ['변이폭', '값 크기가 노드마다 수십 배 달라 **자기 크기에 비례해서** 흔든다'],
        ['후보 수', '(1+λ) 는 부모가 하나라 지역 최적에 갇히기 쉬워 **기본값 8 의 16배**'],
        ['거부한 대안', 'GA — 교차 설계가 실수 벡터에 이득 적음 / 베이지안 — 차원이 높음'],
        ['안전장치', '탐색은 편집 버퍼를 원복. 자산 기록은 명시 버튼에서만'],
      ],
    },
  },
