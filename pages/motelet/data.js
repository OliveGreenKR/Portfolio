...
//      · 에디터 화면 실측 — 탐색 설정 판 90 · 세대 64 · 후보 128
//      ❌ reachCap · w_spawn 류는 SO 자산과 편집 버퍼가 서로 다른 값이다 → 인용 금지.
//      ❌ 특정 구간의 eGold 점프값(예: 51~57판 → 58판)도 인용 금지. 임의로 고른 창 하나라
//         판단 근거가 아니었다. 실제로 본 것은 곡선 전체 모양과 노드별 영향력이다.
...
window.MOTELET_DATA = {
  ...
  // 히어로 3칸 = **만든 것**이지 성과 지표가 아니다.
  //   이 프로젝트에는 개선 전후를 비교할 계측본이 없다. 없는 성과를 숫자로 꾸미지 않는다.
  //   규모·설정값(노드 94 · 곡선 8,192 등)을 여기 올리면 훅이 하는 말과 무관한 숫자가 되므로
  //   본문 안 제자리에서만 쓴다.
  built: [
    { kind: 'MODEL', title: '획득 골드 수학 모델',
      sub: '스킬이 바꾸는 스탯을 한 판의 기대 골드로 환원한다. 런타임 집계 식을 그대로 옮겨 적었다.' },
    { kind: 'TOOL', title: '성장 시뮬레이터 · 에디터 통합',
      sub: '인크레멘탈 루프를 돌려 판 사이 기울기 곡선을 뽑는다. 구간을 집으면 어느 노드가 밀었는지 나온다.' },
    { kind: 'RUNTIME', title: '물리 엔진 없는 배틀 판정',
      sub: '겹침 판정 · 광역 능력 발동 지점 · 화면 점유 기반 스폰 상한을 직접 계산한다.' },
  ],
    ...
    steps: [
      {
        ...
        points: [
          ...
          ['무중복은 맞는 쪽에서',   '같은 적을 두 번 때리지 않는 것을 공격마다 장부로 관리하지 않는다. `Health` 의 I-Frame 이 막는다 — 공격 경로가 몇 개든 규칙은 하나다. 관통이 필요한 공격은 `IgnoresIFrame` 로 **예외를 명시**한다. 뚫리는 자리가 한 곳에 모인다.'],
...
if (!ctx.IgnoresIFrame) _iFrameRemaining = _iFrameDuration;`,
          result: '무중복 보장이 공격 종류 수와 무관해진다. 공격을 추가해도 이 규칙은 안 바뀐다.',
        },
      },
      {
        key: 'density',
        ...
        title: '어디에 터뜨릴 것인가',
        body:
          '광역 능력을 적이 가장 많이 몰린 곳에서 열고 싶었다. ' +
          '"반지름 R 원 하나로 가장 많은 바디를 덮는 지점" 을 찾는 문제이고, 쓴 자료구조는 ' +
          '**표준 균일 격자**(공간 해시)다. 새로 발명한 것은 없다. 정한 것은 그 격자를 이 게임에 어떻게 맞출지다.',
        points: [
          ['셀 한 변을 R 로 맞췄다', '격자를 쓰는 이유가 아니라 *이 크기로* 쓰는 이유다. 셀이 R 이면 반경 R 안의 바디는 반드시 자기 셀의 3×3 안에 들어와, **3×3 만 훑고도** 전수 비교와 같은 답이 나온다. 셀이 더 작으면 이웃을 그만큼 넓게 봐야 한다.'],
          ['후보는 바디 위치 자체',   '격자 칸을 후보로 훑으면 답이 근사가 된다. 최적 원은 거의 항상 어떤 바디에 걸치므로 후보를 바디로 잡았다. 후보 자격은 화면 안으로 제한하되 **세는 대상은 전체**다 — 화면 밖 적도 실제로 맞기 때문이다.'],
          ['격자를 상주시키지 않는다', '대상이 매 프레임 움직인다. 질의 시점에 카운팅 정렬 3패스(셀별 개수 → 누적합 → 배치)로 새로 채운다. 멀리 떨어진 바디 하나 때문에 격자 한 축이 상한을 넘으면 **전수 비교로 폴백**한다 — 답은 같고 비용만 제곱이 된다.'],
          ...
          ['동률은 랜덤 하나',        '가장 많이 덮는 지점이 여럿이면 저수지 샘플링으로 균등하게 하나를 고른다. 적이 흩어진 초반에는 동률이 흔한데, 안 그러면 늘 같은 자리에서 능력이 열린다.'],
        ],
        viz: 'density',
      },
      {
...
성장 체감 (판 i 의 기울기)
  ...
  tool: {
    ...
    readTitle: '두 가지를 읽었다 — 곡선 전체와 노드별 영향력',
    read: [
      ['곡선은 점이 아니라 모양으로', '세로축은 골드의 크기가 아니라 인접한 두 판의 ln 차이다. 한 지점의 값은 의미가 없다 — 보는 것은 **증가율의 추이**다. 어디서 성장이 계단처럼 튀고 어디서 평평해지는지.'],
      ['노드 값을 재는 법 ① 진행형',  '선행 노드만 켠 상태를 기준선으로 두고, 그 노드를 0→1 했을 때 늘어나는 eGold 를 비용으로 나눈다. 이 값이 트리 위에 색으로 얹힌다. 기준선을 만렙으로 두면 데미지가 포화해 무엇을 올려도 eGold 가 거의 안 움직여 **히트맵이 전부 0 에 가까워진다.**'],
      ['노드 값을 재는 법 ② 완전체',  '전 노드를 만렙으로 놓고 **그 노드만 0으로 내려** 잃는 eGold 를 그 노드의 총비용으로 나눈다. 빼 보는 쪽이 더하는 쪽보다 후반 기여를 잘 잡는다.'],
      ['해금 노드의 값이 저절로 잡힌다', '완전체에서 해금 노드를 0으로 내리면 그 능력 채널이 통째로 닫혀 처치 기여가 0이 된다. 그 능력이 벌던 몫 전부가 손실로 잡히므로, **해금의 값을 따로 규칙으로 만들 필요가 없다.**'],
      ['되쓰기',                   '고친 값을 스킬 데이터와 각 설정으로 바로 기록한다. 에디터에서 만지고 그대로 플레이로 확인한다.'],
    ],
    howBody:
      '실제 작업은 이랬다. 수치를 바꾸고 한 판 해본다. 같은 프리셋(시뮬 입력 계수와 노드 수치를 묶어 저장한 것)으로 ' +
      '곡선과 히트맵을 뽑아 옆에 놓는다. 손으로 느낀 과성장 구간과 곡선이 튀어 오르는 구간이 맞아떨어지면, ' +
      '그 구간에서 값이 큰 노드를 히트맵에서 찾아 효과량과 비용을 조정했다. ' +
      '곡선이 답을 준 게 아니라, 플레이에서 받은 인상을 *어느 노드* 로 귀속시킬 수 있게 해줬다.',
    shots: [
      ...
      { src: 'motelet/assets/balance-sim.png',       tag: 'SIM',    caption: '성장 기울기 곡선 전체. 초반의 큰 진폭이 뒤로 가며 평평해지는 모양을 본다. 구간을 집으면 그 구간의 구매 내역도 열린다.' },
