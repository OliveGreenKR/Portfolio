...
window.PACING_DATA = {
  ...
  progress: {
    title: "같은 진행 위치를 기준으로 조건별 영향을 비교한다",
  ...
  sections: [
    {
      ...
      goals: [],
      lead: "시뮬레이션 결과를 검토하고 후보 값을 추가 조정해 원본 자산에 반영했다.",
      prd: [
        {
          kind: "problem",
          label: "문제",
    ...
    },
    {
      ...
      lead: "**완만 성장 → 폭발 성장 → 저항**이 한 스테이지 안에서 여러 번 반복되게 하는 것이 목표였다. 각 조건의 영향을 같은 진행 위치에서 비교하기 위해 **진행도**를 공통 관찰축으로 삼았다.",
      prd: [
        {
          kind: "decision",
          label: "결정",
          text: "등장 몬스터 수, 몬스터 체력, 공격 피해량, 요구 점수는 각각 다르게 움직인다. 각 조건의 영향을 같은 진행 위치에서 비교하기 위해, 레벨과 처치 점수로 정의한 **진행도 = (레벨 − 1) + 처치 점수 ÷ 요구 점수**를 공통 관찰축으로 삼았다. 최초 진입·첫 처치·런당 골드·재통과를 구분해 기록하고, 관측이 없는 구간은 0으로 채우지 않는다."
        },
        {
          kind: "decision",
          label: "결정",
    ...
    },
    {
      ...
      goals: [],
      lead: "**전투와 구매의 게임 루프를 시뮬레이션했다.** 고정된 시간 간격마다 스폰·이동·공격·적중·처치를 갱신하고, **같은 원본 계산 소스**를 Unity 에디터와 CLI가 함께 쓴다.",
      prd: [
        {
          kind: "problem",
          label: "문제",
    ...
    },
    {
      ...
      goals: [],
      lead: "목표와 판정 조건을 직접 정하고 후보를 탐색했다. 선정한 후보의 **HP와 요구 점수를 추가 조정해 2026-09-04 출시 빌드에 반영했다.**",
      prd: [
        {
          kind: "evidence",
          label: "근거",
        ...
        },
        {
          kind: "tradeoff",
          label: "트레이드오프",
          text: "시나리오 데이터에서 후보를 실험하고, 사람이 플레이로 체감을 확인하며 값을 조정했다. 확정한 값은 원본 자산과 출시 빌드에 반영했다."
        }
      ],
      next: null
    }
  ...
  roles: {
    ...
    cards: [
      ...
      },
      {
        ...
        attack: "연쇄번개 · 낙뢰",
        note: "발동 빈도와 한 번에 맞히는 대상 수로 이전 구간의 다수 처치와 골드 획득을 맡는다."
      }
    ],
  ...
  adjust: {
    ...
    rows: [
      ["피해를 더 줘도 타격 수가 줄지 않을 때", "발동 빈도 · 고유 적중 수 · 공간 배치를 본다. 피해와 실제 처치 기회를 분리하기 위해서다."],
  ...
  features: {
    title: "시뮬레이션과 반복 실행의 네 가지 설계 특징",
    items: [
      { icon: "◎", t: "위치에 따라 적중을 판정한다", d: "공격마다 적중 조건이 달라 같은 배치에서도 맞는 마릿수가 갈린다." },
      { icon: "▤", t: "원래 피해와 유효 피해를 구분한다", d: "남은 HP를 넘긴 초과분은 공격별 기여에서 뺀다." },
  ...
  decisions: {
    ...
    rows: [
      ...
      ["편집은 사본에만 쓰고 반영은 따로 실행한다", "원본에 바로 쓰지 않으므로 실험 중 자산이 오염되지 않는다. 삭제 대상으로 지정한 자산은 삭제하지 않고 프로젝트 밖으로 옮긴다."],
...
ApplyOverrides(command, export.Database!, export.Table);`
    ...
    apply: {
      ...
      intro: "에디터에서 사람이 추가한 노드는 사본에만 있다. 반영할 때 원본 폴더에 만들고, 선행 참조를 원본 자산으로 변경한다.",
...
for (int i = 0; i < created.Count; i++) RetargetReferences(created[i], originalByCopy, result);`
  ...
  reach: {
    ...
    rows: [
      ...
      { name: "플레이 확인", state: "confirmed", note: "에디터에서 노드를 편집하고 실제 플레이로 체감을 확인한다." }
    ],
  ...
  cover: {
    ...
    ai: "모델과 실험 도구를 구성하고 후보 검토 기준을 보완했다. 후보 단계의 측정치는 출시 빌드의 성과와 구분한다.",
    links: [
