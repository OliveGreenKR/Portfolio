...
window.PACING_DATA = {
  ...
  // ── 도입부: 전체 시스템 지도 ──────────────────────────────────
  // 위층 = 밸런스 대상인 게임, 아래층 = 그것을 다루려고 만든 것.
  // 두 층을 잇는 관계는 «읽는다»와 «바꾼다» 둘뿐이다 —
  // 게임 인과와 도구의 데이터 흐름을 같은 화살표로 섞지 않는다.
  system: {
    label: "전체 구조",
    title: "밸런스 대상과, 그것을 다루려고 만든 것",
    gameLabel: "게임 — 밸런스 대상",
    game: [
      { id: "asset", t: "몬스터 · 레벨 자산", d: "HP · 보상 · 요구 점수 · 출현 구성" },
      { id: "battle", t: "전투", d: "공격 · 적중 · 처치" },
      { id: "reward", t: "골드 · 처치 점수", d: "구매와 레벨로 갈라짐" },
      { id: "growth", t: "구매 · 레벨", d: "다음 런의 전투 조건을 바꿈" }
    ],
    toolLabel: "만든 것",
    tool: [
      { id: "model", t: "판정 모델", d: "역할 · 타격 수 · 공간 · 변수 책임" },
      { id: "sim", t: "런 시뮬레이터 · 캠페인", d: "고정 틱 상태 갱신, 구매를 이어 반복" },
      { id: "core", t: "공유 계산 코어 · CLI", d: "사본 없이 원본 소스를 컴파일" },
      { id: "read", t: "결과와 분석", d: "진행도 페이스 · 기여도" },
      { id: "human", t: "사람의 판단", d: "후보 선택과 마무리" }
    ],
    linkDown: "자산을 읽는다",
    linkDownNote: "에디터가 시나리오로 내보냄",
    linkUp: "자산을 바꾼다",
    linkUpNote: "판단한 값을 사람이 반영",
    caption: "위층은 게임 안의 인과, 아래층은 도구의 데이터 흐름입니다. 두 층을 잇는 관계는 «읽는다»와 «바꾼다» 둘뿐이며, 도구가 자산을 직접 고치지 않습니다."
  },
  ...
  goal: {
    title: "목표는 하나의 주기입니다",
    figTitle: "저항 → 고속 성장 → 완속 성장, 그리고 다시 저항",
    repeat: "한 스테이지 안에서 여러 번 반복",
    cycle: [
      {
        id: "resist", name: "저항",
        d: "새 레벨의 적을 아직 안정적으로 잡지 못한다.",
        beats: ["신레벨의 저항", "첫 진입 · 불안정 사냥"]
      },
      {
        id: "fast", name: "고속 성장",
        d: "처형으로 첫 돌파, 몸통·크기 상승, 이전 티어 압축, 밀도 보강이 이어진다.",
        beats: ["처형 첫 돌파 · 몸통 상승 · 압축 · 밀도 보강", "정착 · 하위 압축 · 밀도·지속 보강"]
      },
      {
        id: "slow", name: "완속 성장",
    ...
    beatSources: ["지침", "모델"],
    sourceNote: "설계 문서 두 개가 같은 주기를 각각 적습니다. «지침»은 Stage04 재설계 실행 지침의 박자, «모델»은 판정 모델 문서의 레벨 주기입니다. 3단은 그 둘을 묶어 읽은 것이며 원문의 단어가 아닙니다.",
    noveltyTitle: "반복이 지겨워지지 않게 하는 장치",
    novelty: "역할 격자에서 **어느 계열의 성장 노드가 비어 있는 것은 누락이 아니라 다른 계열에 존재감을 넘기는 수단**입니다. 그리고 노드는 타격 수 · 압축 · 활동시간 · 화면 생존 수 · 크기비 · 구매 순서 · 보스 시도당 HP 감소량 중 **적어도 하나의 관찰 가능한 경계를 바꿀 때만** 존재 가치가 있습니다.",
    axesTitle: "그래서 판정 축을 둘로 좁혔습니다",
    axes: [
      { k: "축 1", t: "진행도 페이스", d: "진행도 = (레벨 − 1) + 처치 점수 ÷ 요구 점수. 이 축 하나 위에서 페이스가 목표대로 흐르는가." },
      { k: "축 2", t: "기여도 교대", d: "존재감이 한 계열에 고이지 않고 계속 옮겨 가는가." }
    ],
    axesNote: "이 두 축이 그대로 도구의 출력이 됩니다 — §07의 화면 두 장이 각각 이 축입니다.",
    badgeLabels: { resist: "저항", fast: "고속 성장", slow: "완속 성장", novelty: "새로움" },
    badgeLegend: "이 절이 다루는 주기 구간",
    mine: "목표를 세운 것은 본인이고, 그것을 계산 가능한 기준으로 옮긴 것이 아래 내용입니다."
  },
  ...
  sections: [
    {
      ...
      title: "목표는 하나의 주기입니다",
      goals: [],
      lead: "한 번의 전투는 보스 처치나 스태미나 소진으로 끝나고 레벨은 처음으로 돌아갑니다. 넘어가는 건 **구매한 능력뿐입니다.** 그 반복 위에 **저항 → 고속 성장 → 완속 성장**이 여러 번 돌아오게 하는 것이 목표였습니다.",
      prd: [
        {
          kind: "problem",
          label: "문제",
        ...
        },
        {
          kind: "decision",
          label: "결정",
          text: "골드와 **처치 점수(XP)**를 서로 다른 자원으로 나눠 봤습니다. 처치 점수는 런 안에서만 쓰이고 레벨을 올린 뒤 사라집니다. 골드는 런이 끝난 뒤 능력 구매로 이어져 다음 런의 전투 조건을 바꿉니다. 둘을 «성과» 하나로 합치면 «이번 판을 잘했다» 와 «다음 판이 쉬워졌다» 를 구분할 수 없습니다."
        },
        {
          kind: "decision",
          label: "결정",
          text: "주기를 재려고 지표를 늘리는 대신 **판정 축을 둘로 좁혔습니다** — 진행도 페이스 하나, 기여도 교대 하나. 탐색 중에는 구간별 목표 시간 비중이나 역할별 기여비 같은 값도 뒀지만, 그건 후보를 고르는 동안 쓴 눈금이고 **최종적으로 남은 기준은 이 둘입니다.**"
        }
      ],
      next: "그 저항은 어디서 오는가?"
    },
    ...
    },
    {
      ...
      prd: [
        ...
        },
        {
          kind: "evidence",
          label: "근거",
          text: "격자의 **빈칸이 장치입니다.** 어느 계열에 그 구간의 성장 노드가 없다는 것은 누락이 아니라 **다른 계열에 존재감을 넘기는 수단**이라고 판정 모델이 정의합니다. §01에서 말한 «새로움의 공급»이 여기서 실제 형태로 회수됩니다 — 존재감이 옮겨 다니게 만드는 자리가 곧 빈칸입니다."
        },
        {
          kind: "tradeoff",
          label: "트레이드오프",
    ...
    },
    {
      ...
      title: "도구가 낸 값을, 사람이 마감했습니다",
      lead: "목표를 직접 세우고 → 탐색을 시키고 → 판정 조건으로 후보를 고르고 → **그 값을 그대로 쓰지 않고 손으로 마무리해** 빌드에 반영했습니다.",
      prd: [
        {
          kind: "evidence",
          label: "근거",
          text: "자산을 바꾼 커밋이 그 순서를 기록합니다. 후보 탐색이 끝난 뒤 **두 번 더 손을 댔고**, 그래서 지금 빌드의 HP 사다리와 요구 점수는 후보 보고서의 값과 다릅니다. 도구가 답을 냈고, 그 답을 채택할지와 어디를 고칠지는 사람이 정했습니다."
        },
        {
          kind: "decision",
          label: "결정",
          text: "이렇게 닫는 것이 처음부터의 규칙이었습니다. 착수 시점의 채택 조건에 **«실플레이 검증 전에는 시각적 체감과 실제 선택 수렴을 확정 사실로 표현하지 않는다»**가 들어 있었습니다. 이 페이지에 성능 수치와 개선률이 없는 것은 결과가 나빠서가 아니라 그 조건을 지켰기 때문입니다."
        },
        {
          kind: "tradeoff",
          label: "트레이드오프",
          text: "**사본에서 실험 · 자산 반영 · 빌드 반영 · 실플레이 검증은 서로 다른 상태입니다.** 확인은 빌드 반영까지이고, 실제 플레이로 체감을 검증한 기록은 아직 없습니다. 후보 단계의 측정치를 지금 빌드의 성과로 옮기지 않습니다."
        }
      ],
      next: null
    }
  ...
  reach: {
    title: "네 상태는 서로 다릅니다",
    rows: [
      { name: "사본에서 실험", state: "confirmed", note: "모델과 CLI로 후보를 반복 실행하고 판정 조건으로 골랐습니다." },
      { name: "자산 반영", state: "confirmed", note: "Stage04 자산을 바꾼 커밋 네 건이 남아 있고, 마지막 두 건이 후보 이후의 손조정입니다." },
      { name: "빌드 반영", state: "confirmed", note: "그 값이 빌드에 들어갔습니다. 게임은 개발 중이며 출시 전입니다." },
      { name: "실플레이 검증", state: "open", note: "체감을 실제 플레이로 확인한 기록은 아직 없습니다." }
    ],
