...
window.PACING_DATA = {
  ...
  results: {
    ...
    boundary: "개발 중인 도구의 실제 화면이다. 강제 클리어가 포함된 실행이라 화면의 수치는 관찰 기능의 예시이며, 자연 완주나 개선 성과로 해석하지 않는다.",
  ...
  sections: [
    ...
    },
    {
      ...
      prd: [
        ...
        },
        {
          kind: "tradeoff",
          label: "트레이드오프",
          text: "화면의 수치는 **관찰 기능을 설명하기 위한 예시**다. 강제 클리어가 포함된 개발 중 실행이므로 자연 완주나 개선 성과로 인용하지 않는다."
        }
      ],
    ...
    },
    {
      ...
      prd: [
        ...
        },
        {
          kind: "tradeoff",
          label: "트레이드오프",
          text: "사다리의 값은 **설계된 격차의 모양**이지 플레이어가 실제로 번 골드가 아니다. 출현 가중치는 스폰 선택 확률이고, 역할 배분과 1타 목표 범위는 설계 목표이며 특정 구매 시점의 달성값이 아니다."
        }
      ],
    ...
    },
    {
      ...
      goals: [],
      lead: "목표와 판정 조건을 직접 정하고 후보를 탐색했다. 선정한 후보의 **HP와 요구 점수를 추가 조정해 빌드에 반영했다.** 실제 플레이에서의 체감 검증은 남아 있다.",
      prd: [
        {
          kind: "evidence",
          label: "근거",
          text: "후보 선정 후 **두 차례 추가 조정**했다. 현재 빌드의 HP와 요구 점수는 후보 보고서의 값과 다르며, 자산 변경 커밋에 이 과정이 남아 있다."
        },
        {
          kind: "decision",
          label: "결정",
        ...
        },
        {
          kind: "tradeoff",
          label: "트레이드오프",
          text: "**시나리오 데이터에서 실험 · 자산 반영 · 빌드 반영 · 실플레이 검증**을 구분했다. 적용은 빌드 반영까지 완료했으며, 실제 플레이로 체감을 검증한 기록은 아직 없다."
        }
      ],
      next: null
    }
  ...
  ladder: {
    ...
    caption: "2026-09-06 확인한 Stage03 자산 설정값이다. 보유 효과를 적용하기 전 기본값이며 최종 출시 수치가 아니다. 로그 눈금 한 칸은 100배 차이를 나타낸다."
  },
...
for (int i = 0; i < created.Count; i++) RetargetReferences(created[i], originalByCopy, result);`
  ...
  reach: {
    ...
    rows: [
      ...
      { name: "빌드 반영", state: "confirmed", note: "그 값이 빌드에 들어갔다. 게임은 개발 중이며 출시 전이다." },
      { name: "실플레이 검증", state: "open", note: "체감을 실제 플레이로 확인한 기록은 아직 없다." }
    ],
  ...
  evidence: {
    ...
    boundary: "개발 중인 도구의 실제 화면이다. 강제 클리어가 포함된 실행이라 수치는 관찰 기능의 예시이며, 자연 완주나 개선 성과로 해석하지 않는다.",
    timeline: {
  ...
  cover: {
    ...
    ai: "모델과 실험 도구를 구성하고 후보 검토 기준을 보완했다. 실제 재미와 출시 성과는 별도 검증 대상이다.",
    links: [
      { label: "Steam", v: "Motelet (개발 중)", href: "https://store.steampowered.com/app/4850970/Motelet/", tone: "blue" }
    ],
    caption: "개발 중 전투 화면이다. 골드는 구매에, 처치 점수는 레벨 진행에 쓰인다."
  },
