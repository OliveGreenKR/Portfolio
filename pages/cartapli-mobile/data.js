...
window.CM_DATA = {
  ...
  meta: {
    ...
    headline: {
      ...
      k: '접는 동안 프레임 하나에 드는 세 마커(`Sync` · `Split` · `Compose`)의 합 · S0 대비 −93.8%',
      href: '#result',
  ...
  s2: {
    ...
    facts: [
      ...
      ['얼리기', '단계마다 그 시점 엔진을 브랜치로 얼려 둔다 — 태그 1개(`perf-s0`)와 브랜치 4개(`bench-s0` ~ `bench-s2-a`). 얼린 브랜치에 덮어쓰는 것은 **로깅 호출부의 API 이관과 프로파일러 마커뿐**이고 접기 로직은 손대지 않는다. 벤치 코드는 최신 것 하나를 그대로 얹는다. 그래서 한 달 뒤에도 **같은 자극으로 다시 잴 수 있다** — 09 가 실제로 그렇게 했다.'],
  ...
  s3: {
    ...
    lead: '아래 다섯 막대는 얼려 둔 브랜치를 **같은 벤치로 다시 돌려** 잰 값이다. 어떻게 다시 잴 수 있었고 왜 다시 쟀는지는 09 가, 각 단계가 무엇을 고친 것인지는 05~08 이 답한다.',
    chart: {
      unitNote: 'S0 = 100 기준',
      legend: [
  ...
  s4: {
    ...
    lead: '막대 네 칸이 각각 무엇이었는지를 한 장에 놓는다. 가로축은 **프레임 두 종류**다 — 접는 선을 끄는 동안(보간)과 놓는 한 프레임(확정). 뒤에 붙는 09·10 은 단계가 아니라 후속이라 아래쪽에 따로 둔다.',
    map: {
      ...
      rows: [
        ...
        { n: '③', side: 'lerp', title: '레이어당 렌더 오브젝트를 없앤다', what: '레이어 한 장마다 붙던 한 벌' },
    ...
    vizCaption: '①③④ 는 **선을 끄는 동안 매 프레임 도는 비용**이고, ② 만 **확정하는 한 프레임**의 비용이다. 아래 두 칸은 단계가 아니라 그 뒤에 온 것이라 막대에 들어가지 않는다.',
  },

  /* ── S5 진단 (Level 2) — §02 M2′ ─────────────────────────────────────── */
  s5: {
    gist: '예상은 폴리곤 분할이었고, 실측은 렌더링 준비였다.',
    // ⚠️ 첫 문장이 03 의 지도를 명시적으로 가리켜 03↔04 순서를 잠근다(05 렌즈2: G2 미통과 원인)
    body: '지도의 네 칸이 **왜 그 넷이고 왜 그 순서인지**가 여기서 정해졌다. 회차를 거듭할수록 레이어가 쌓이니 자르는 계산이 무거워졌으리라 보고 폴리곤 분할부터 의심했는데, 마커를 나눠 재 보니 순서가 반대였다.',
    ...
    caption: '예상은 분할이었다. 실측에서는 렌더링 준비가 **2.7배** 비쌌다. 셋을 더한 0.643 ms 가 이 페이지의 대표 지표이고, **이 표는 09 의 재측정 뒤에 다시 낸 값**이다.',
    ...
    after: '정체는 기하 계산량이 아니었다. 같은 회차의 정점 총수는 1,348개인데 레이어는 337장이었고, 비용은 레이어 **한 장마다 붙는 렌더 오브젝트** 쪽에 있었다. 그래서 GPU 문제로 넘기지 않고 CPU 쪽으로 방향을 잡았다 — GPU 는 가려진 픽셀을 z-buffer 로 버리지만, CPU 는 화면에 보이지도 않을 레이어의 메시까지 매번 새로 만들고 있었다.',
  },
  ...
  s6: {
    ...
    pair: {
      ...
      intro: '뷰가 "지난 프레임과 같은가" 를 물으려면 근거가 있어야 하는데, 연산이 매번 새 객체를 뱉으니 물어볼 수가 없었다. 그래서 뷰가 아니라 연산의 반환 규약을 먼저 고쳤다. 긴 인자 목록은 지면에서 줄였고 그 자리를 … 로 표시했다.',
      before: {
...
if (side == PaperGeometry.PolygonSide.Back) { … }`,
    ...
    notes: [
      '고친 자리는 뷰가 아니라 **연산의 반환 규약**이다. 뷰가 물어볼 근거를 연산이 먼저 만들어 줬다. `PaperLayer` 는 불변이라 참조가 같으면 내용도 같고, 그래서 참조 비교가 정당한 근거가 된다.',
...
if (keptCount == _layers.Count) return this;`,
    ...
    notes: [
      ...
      '이 시점의 가려짐 판정은 범용 폴리곤 불리언(`Clipper2 유니온`)이었다. 그 방식을 바꾸는 것은 한참 뒤인 **10** 이다.',
    ],
  },
...
_paperRenderer.Sync(_pipeline.View);`,
  ...
  s10: {
    ...
    split: '`Renderer.Sync` 가 왜 비싼지 모르는 채로 범인을 짚지 않았다. 먼저 `Sync` 안을 `Fill` · `Upload` · `Log` 세 마커로 쪼갰다. 재 보니 `Fill` 과 `Upload` 를 합쳐도 `Sync` 의 10% 였다. 남은 90% 는 프레임마다 한 줄 도는 `Debug.Log` 였다 — 고치기 전 마지막 단계의 **후반 회차 r13~16 보간 프레임 중앙값** 229,300ns 중 205,900ns, **89.8%.** 레이어가 많은 초기 단계에서는 비중이 이보다 낮다(S0 에서 약 28%).',
    pair: {
      file: 'PaperRenderer.cs — 로그 게이트',
      commit: 'f920035 · 2026-07-24',
      intro: '게이트가 로그 함수 안에 있으면 이미 늦다. C# 은 인자를 먼저 다 계산하므로, 로그가 꺼져 있어도 보간 문자열은 만들어진다. 로그 문자열과 LogIf 본문의 전역 스위치 한 줄은 지면에서 줄였고 그 자리를 … 로 표시했다.',
      before: {
...
static readonly ProfilerMarker LogMarker    = …;`,
      ...
      result: '이 한 줄이 마지막 단계 후반 회차에서 Renderer.Sync 비용의 89.8% 였다',
    },
    notes: [
  ...
  s11: {
    ...
    vizCaption: '한 조각을 **덮개**(그 위에 쌓인 레이어 하나)로 빼는 그림이다. 덮개가 볼록이면 **변마다 한 번씩 자르는 것**으로 나머지가 정확히 구해진다 — 이번 변 바깥은 조각으로 떼어 내고 안쪽만 다음 변으로 넘기면 되므로 **교차점을 따로 구할 필요도 도형을 다시 조립할 필요도 없다.** 남은 것이 없으면 그 레이어는 완전히 가려진 것이다.',
    pair: {
      file: 'PaperVisibility.cs → PaperCoverage.cs',
      commit: '0d73c39 로 갈아엎고 7943d04 로 다듬음 · 2026-08-12',
      intro: 'Clipper2 유니온은 범용 폴리곤 불리언으로 덮개를 합집합으로 쌓고 차집합을 뺀다. 볼록 뺄셈은 덮개의 변을 차례로 반평면 경계로 삼아 자르기만 반복한다. 뒷쪽 발췌는 다듬은 뒤(7943d04) 코드다.',
      before: {
        ...
        code: `for (int e = 0; e < coverCount; e++)
{
    ...
    notes: [
      ...
      '조각 수는 원리적으로 폭발할 수 있어(덮개가 조각 한가운데에 들어가면) 작업 버퍼를 고정 크기로 두고 **넘치면 "안 가려짐"으로 물러선다**(위 `MaxPieces` 가드). 판정이 곧 삭제 허가라 **두 오류의 값이 비대칭**이기 때문이다 — 가려진 걸 놓치면 레이어 한 장이 남을 뿐이지만, 안 가려진 걸 가려졌다고 하면 종이가 화면에서 사라진다.',
    ],
    table: {
    ...
    tablecap: '두 수치 열은 **더할 수 없다** — 왼쪽은 16회 누적, 오른쪽은 회차 16 시점의 잔량이다. `단일 덮개`의 누적은 남기지 않았다. ⚠️ **`Clipper2 유니온` 행의 57장만 출처가 다르다** — 그 방식이 실제 런타임이던 2026-07-24 벤치에서 온 값이고, 나머지 두 행은 아래 하네스에서 나왔다. ⚠️ 그리고 이 방식이 더 느린데도 덜 버리는 이유는 **누적 합집합을 한 장씩 눌러 담는 과정에서 얇은 조각이 유실되기 때문**으로 보이나, 원인을 따로 재지 않았다 — 그래서 12~23배라는 배수도 그 구현을 기준으로 한 값이다.',
  ...
  /* ── S12 검증과 한계 (Level 4) ───────────────────────────────────────── */
  s12: {
    lead: '**여섯 번 갈아치웠으면** 다음 질문은 그 숫자들이 같은 그림에 대한 것인가다. 특히 08 에서 분할 구현을, 10 에서 판정 방식을 통째로 바꿨다.',
    ...
    limits: [
      ['측정 환경', '전부 Windows PC · Unity 에디터 PlayMode 다. **하드웨어 사양은 기록하지 않았다.** 기기 실측과 GPU 시간도 없다. 전후 비교가 성립하는 근거는 하드웨어가 아니라 모든 단계를 같은 절차로 다시 쟀다는 것이다.'],
    ...
    close: '다음 한 줄은 이 목록의 맨 위를 지우는 일이다 — development build 로 기기에서 한 번 재면 위 넷 중 셋이 닫힌다.',
  },
};
