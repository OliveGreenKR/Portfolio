...
window.CM_DATA = {
  ...
  s2: {
    ...
    facts: [
      ['프로젝트', 'PC 원작 `Cartapli` 의 모바일 이식 + 리메이크. 퍼즐 게임이고 **개발 중**이다. 원작과는 **별개 레포**이고 커밋 122개다.'],
  ...
  s5: {
    ...
    after: '정체는 기하 계산량이 아니었다. 같은 회차의 정점 총수는 1,348개인데 레이어는 337장이었고, 비용은 레이어 **한 장마다 붙는 렌더 오브젝트** 쪽에 있었다. 그래서 GPU 문제로 넘기지 않고 CPU 쪽으로 방향을 잡았다 — GPU 는 가려진 픽셀을 z-buffer 로 버리지만, CPU 는 화면에 보이지도 않을 레이어의 메시까지 매번 새로 만들고 있었다. **지도에서 ①③④ 가 보간 프레임 쪽에 몰려 있는 것이 이 진단의 결과다** — 비싼 것이 거기였다.',
  },
...
if (keptCount == _layers.Count) return this;`,
  ...
  s8: {
    ...
    pair: {
      ...
      intro: '레이어 수가 늘어도 객체 수는 안 늘게 만든다.',
      before: {
...
static readonly ProfilerMarker LogMarker    = …;`,
    ...
    notes: [
      '**범인을 짚기 전에 계측을 먼저 쪼갰다** — 세 마커로 나눠 재고 남은 90% 가 어디인지 그림이 가리키게 한 뒤에 그 한 줄을 찾았다.',
        ...
        code: `for (int e = 0; e < coverCount; e++)
{
  ...
  s12: {
    ...
    close: '다음 한 줄은 기기에서 한 번 재는 일이다. development build 프로파일 한 번이면 **측정 환경 · 드로우콜 · 오버드로우** 셋이 닫힌다 — 나머지는 각각 다른 실험이 필요하다.',
  },
};
