...
//    가려짐 계산이 아직 `합쳐서 빼기` 이던 상태. s11 = 2026-08-12 / 별도 하네스.
...
//   가려짐 계산 `합쳐서 빼기` / `한 장씩 빼기`(채택) / `한 장씩만 본다` — 지면에는 이 세 이름만 쓴다.
...
window.CM_DATA = {
  ...
  meta: {
    ...
    headline: {
      rows: [
        { k: '접는 동안 프레임 하나에 드는 시간', a: '0.643 ms', b: '0.040 ms', d: '−93.8%' },
        { k: '종이를 화면에 올리는 데 쓰는 유니티 객체', a: '337개', b: '2개', d: '레이어가 몇 장이든 고정' },
        { k: '드로우콜 (가만히 있을 때 대비)', a: '+298', b: '+1', d: '' },
        { k: '접기를 확정하는 순간 생기는 쓰레기', a: '1.73 MB', b: '~28 KB', d: 'GC 수집 0회' },
      ],
      href: '#result',
  ...
  s3: {
    ...
    notes: [
      ...
      '⚠️ 다섯 전부 **2026-07-24 에 잰 값**이고, 그때 가려짐 계산은 아직 `합쳐서 빼기` 였다. §10 에서 바꾼 방식은 그 뒤(2026-08-12)라 이 막대에 안 들어 있다. 그리고 이 축은 프레임당 Average 라 초반 저부하 프레임이 표본의 다수를 차지한다 — 후반의 레이어 격차는 §07 의 구조 표로 봐야 한다.',
    ],
  },
...
for (int i = 0; i < data.LayerCount; i++)
{
    ...
    reconcile: '위 두 행은 **버리기가 먼저 4분의 3을 하고** 병합이 나머지를 장수와 무관하게 만든다. 아래 두 행은 버리기만의 몫이다 — 병합은 그리는 방식만 바꾸므로 데이터가 달라지면 안 되고, 실제로 안 달라졌다. **77 과 57 이 다른 것**은 유니티 객체를 담아 두는 풀이 접는 동안의 최대치까지 커진 뒤 도로 줄지 않기 때문이고, **드로우콜이 렌더 오브젝트보다 조금 적은 것**은 같은 머티리얼끼리 일부가 배칭되기 때문이다. 2026-07-24 에 잰 값이고 가려짐 계산은 아직 `합쳐서 빼기` 였다(§10 에서 바꾼 뒤에는 41장이 남는다). ①의 열이 없는 것은 그 변경이 이 지표들을 안 움직이기 때문이다.',
  },
...
_paperRenderer.Sync(_pipeline.View);`,
    ...
    results: [
      '자르는 계산 **0.0403 → 0.0176 ms** (−56.3%). ⚠️ 2026-07-24 · 가려짐 계산이 아직 `합쳐서 빼기` 이던 때의 값이다 — §10 에서 바꾼 뒤 같은 구간을 다시 재면 0.0121 ms 다.',
...
static readonly ProfilerMarker LogMarker    = …;`,
  ...
  s11: {
    ...
    ways: [
      ['합쳐서 빼기', '위에 쌓인 레이어를 **전부 하나로 합친 다음** 아래 조각에서 뺀다. 아무 모양이나 다룰 수 있는 범용 라이브러리(`Clipper2`)가 필요하다.', '정확하다'],
      ['한 장씩 빼기', '합치지 않고 **한 장씩 차례로** 뺀다. 빼다가 남는 것이 없어지면 그 레이어는 가려진 것이다.', '정확하다 · **채택**'],
      ['한 장씩만 본다', '레이어 **하나가 통째로** 아래를 품는 경우만 본다. 여러 장이 나눠 가리는 건 못 잡는다.', '놓치는 경우가 있다'],
    ],
    compareCaption: '위에 **두 장이 걸쳐** 있고 **둘이 합쳐야** 아래 조각을 다 가리는 상황이다. `한 장씩만 본다`는 이걸 못 잡아 조각을 못 버리고, 나머지 둘은 잡는다. **답이 같으면 남는 문제는 어떻게 구하느냐**다.',
    vizCaption: '채택한 `한 장씩 빼기`가 실제로 어떻게 도는가. 조각 하나에서 **덮개**(위에 쌓인 레이어 한 장)를 빼는 그림이다. 덮개가 볼록이면 **변마다 한 번씩 자르는 것**으로 나머지가 정확히 구해진다 — 이번 변 바깥은 조각으로 떼어 내고 안쪽만 다음 변으로 넘기면 되므로 **교차점을 따로 구할 필요도 도형을 다시 조립할 필요도 없다.**',
    pair: {
      file: 'PaperVisibility.cs → PaperCoverage.cs',
      commit: '0d73c39 로 갈아엎고 7943d04 로 다듬음 · 2026-08-12',
      intro: '합쳐서 빼기는 범용 라이브러리로 위 레이어들을 하나로 합친 뒤 차집합을 구한다. 한 장씩 빼기는 덮개의 변을 차례로 경계선 삼아 자르기만 반복한다. 아래 발췌는 다듬은 뒤(7943d04) 코드다.',
      before: {
        ref: '합쳐서 빼기 — 합집합을 쌓고 차집합을 구한다',
        ...
        code: `for (int e = 0; e < coverCount; e++)
{
    ...
    notes: [
      '`합쳐서 빼기`를 뺀 진짜 근거는 속도가 아니라 **도형의 성질**이다. 처음 종이가 볼록(오목한 데가 없는 모양)이고 자르기가 볼록을 볼록 둘로만 가르므로, 아무리 접어도 **레이어는 볼록에서 벗어나지 않는다.** 그러면 아무 모양이나 다루는 범용 라이브러리가 **애초에 필요 없다** — 직선으로 자르기만 반복하면 되고, 근사가 아니라 정확하다.',
    ...
    table: {
      ...
      rows: [
        ['합쳐서 빼기', '채택한 것의 **12~23배**', '69장', '57장', '기각 — 느린데 버린 장수도 더 적다'],
        ['한 장씩 빼기', '기준', '98장', '41장', '**채택**'],
        ['한 장씩만 본다', '채택한 것의 **0.37~0.45배**', '기록 없음', '57장', '기각 — 계산은 싸지만 뒤가 따라 오른다'],
      ],
    },
    tablecap: '가운데 두 열은 **더할 수 없다** — 왼쪽은 16회 동안 버린 누적이고 오른쪽은 16회차 시점에 남은 수다. `한 장씩만 본다`의 누적은 기록하지 않았다. **합쳐서 빼기가 더 느린데도 덜 버리는 이유**는, 채택한 쪽이 아주 얇게 남은 잔여 조각까지 오차 범위로 삼켜 더 버리기 때문이다. ⚠️ 첫 행의 **57장만 출처가 다르다** — 그 방식이 실제로 돌던 2026-07-24 벤치 값이고 나머지는 아래 조건에서 잰 값이다.',
    decide: '**승패를 가른 것은 계산 속도가 아니라 접는 빈도였다.** `한 장씩만 본다`는 계산이 싸지만 덜 버린다. 남는 레이어가 41장에서 57장으로 늘면 그만큼 자르기와 화면 올리기가 매 프레임 따라 오른다. 그 **매 프레임 손해**가 채택한 쪽의 **확정 한 번 손해**를 갚는 데 19프레임(60fps 에 0.32초)이 걸리는데, 실제 게임은 20~30초에 한 번 접으니 그보다 60~100배 뜸하다 — 그 간격으로 나눠 붙이면 프레임 평균이 38.5µs 대 45.9µs 다. ⚠️ 이 20~30초는 **설계값이지 실제 플레이를 잰 값이 아니고, 선택 전체가 그 값 위에 서 있다.** 그래서 **되돌리는 조건을 코드와 함께 남겼다** — 접기 횟수 상한이 5회 이하로 정해지거나 간격이 0.3초 아래로 잦아지면 `한 장씩만 본다`로 돌아간다. 그쪽 구현을 지우지 않았고 테스트가 셋을 다 돈다.',
  },
