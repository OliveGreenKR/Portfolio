...
//   → S6 ①재사용(L3) → S7 ②버리기(L3) → S8 ③병합(L3) → S9 ④Burst(L3)
//   → S10 ⑤재측정(L3) → S11 ⑥판정(L3) → S12 검증·한계(L4)
//
// 순서의 뼈대는 커밋 날짜다 — 385fd68(7/22) · 992c50a(7/23) · 792bb3a(7/24) ·
// cafa4ae(7/24) · f920035(7/24) · 0d73c39(8/12).
// ⚠️ 단 S8·S9·S10 은 **셋 다 7/24 라 날짜가 순서를 못 잠근다.** 그 구간을 잠그는 것은
//    각 절 첫 줄(`link`)의 후방참조뿐이다 — "②가 버린 뒤에도 77개" · "렌더 쪽을 끝내고 나면"
//    · "넷을 다 고치고 다 잰 뒤". 이 문장들을 지우면 그 구간의 순서가 풀린다.
// 잠금이 설계에만 있고 지면에 안 적혀 있으면 독자에게는 안 잠긴다(직전 회차 G2 미통과 원인).
//
// S6~S11 은 고정 리듬이다 — [link] → [기존→개선 그림] → [코드 쌍] → [설명 3줄].
// 그 뒤에 절별 부록(표·수치 목록)이 붙는 절이 있다 — 계약이 각 절에 배정한 것이고 리듬 밖이다.
