...
//   1. **성능 주장 금지.** 프로파일러 측정본이 하나도 없다.
//      버린 것 — "15 → 60 fps"(영상 기억, 조건 없음) · "터널링 안전속도 5 → 25 m/s"(근거 없음)
...
static_assert(std::is_base_of_v<UGameObject, T> ||
  ...
  limits: [
    ['프로파일러를 돌린 적이 없다',
     '판정 수단은 화면에 띄운 fps 카운터와 눈으로 본 관통 여부였다. 같은 PC · 같은 빌드에서 전후를 비교하긴 했지만 씬 구성도 객체 수도 남기지 않아, 그때 본 수치는 이 페이지에 싣지 않았다.'],
