...
// 6. handoff 는 §02→§03 한 곳만. 나머지 절은 앞뒤로 인과가 없다.
// 7. **작업 편의를 성과로 팔지 않는다.** 값을 한 곳에 모았다 · 계산부를 재생 없이 돌린다 ·
//    일찍 했다 — 전부 뺐다. 읽는 쪽이 살 이유가 없다. 그 자리에 있던 설계 판단 하나
//    (두 힘의 크기를 일부러 안 맞췄다)만 §02 요점으로 옮겼다.
...
window.ML_DATA = {
  ...
  built: [
    {
      kind: 'FORCE',
      title: '힘을 한 곳에서 더한다',
      sub: '각 다리는 자신의 힘만 전달하고, 몸통은 이를 합산해 물리 시스템에 한 번 적용한다. 몸통의 이동 방향은 별도 입력이 아니라 합력의 결과로 결정된다.',
    },
    {
      kind: 'SPLIT',
      title: '다리는 서로를 모른다',
    ...
    {
      kind: 'FEEL',
      title: '물리보다 감각을 먼저 뒀다',
      sub: '뻗는 힘과 당기는 힘의 크기를 일부러 맞추지 않았다. 실제 물리와 어긋나지만 만들려는 감각이 나온다.',
    },
  ],
  ...
  context: {
    ...
    tension: [
      ...
      ['몸은 하나다',
       '결국 몸통은 한 방향으로만 움직인다. 그 방향을 누가 정하느냐가 이 PoC의 물음이다.'],
    ],
    tensionWhy:
      '검증 기준은 하나였다. **여러 힘이 합벡터로 모이는 구조가 실제 조작으로 성립하는가.**',
    ...
    scope: {
      ...
      why:
        ...
        '**IK도 이 PoC가 만든 것이 아니다** — 관절을 굽히는 계산은 표준 알고리즘을 썼고, 정한 것은 그 위의 구조다.',
    },
  },
  ...
  force: {
    ...
    body3:
      '몸통의 이동 방향은 별도 입력이 아니라 세 힘의 합으로 결정된다. 힘이 같은 방향이면 이동하고, 어긋나면 제자리에서 흔들린다.',

    points: [
      ['힘은 두 종류다',
       '뻗을 때 생기는 반작용과, 붙잡고 나서 당기는 힘. 방향도 생기는 시점도 다르다.'],
      ['두 힘의 크기를 일부러 안 맞췄다',
       '작용과 반작용을 같게 두면 "다리는 잘 뻗는데 몸이 안 끌려온다" 같은 감각을 만들 수 없다. 실제 물리와 어긋나는 것을 알고 택했다.'],
      ['한 번에 한 다리만 조종한다',
    ...
    handoff: {
      ...
      a: '답은 **각 구성요소가 서로를 몰라도 된다**는 것이었다.',
    },
  },
  ...
  split: {
    ...
    body3:
      '그 비용을 하루짜리 PoC에서 미리 치르기로 하고, 다리 하나를 처음부터 독립된 조각으로 두었다.',
