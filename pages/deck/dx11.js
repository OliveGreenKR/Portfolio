...
//   02 물리   구조(소유물과 네 통로)       → 흐름(한 틱의 순서) → 세부(입력 동기화 · 충돌)
...
// cover → 범위·근거·한계
//       → [엔진 구조] → [엔진 흐름]
//       → [물리 구조] → [물리 흐름] → 흐름 code → 입력 동기화 3계층 → 입력 동기화 code → 충돌
//       → [렌더 구조·흐름] → 세부 code
//
// Removed:
...
(function buildDX11Deck() {
  ...
  window.DECK_PARTS.dx11 = {
    proj: 'DX11 엔진 제작',
    slides: [
      // 표지는 **프로젝트가 소유한다** — pages/dx11-engine/cover.jsx.
      // 같은 표지를 덱 · 랜딩 카드 · 상세 페이지 히어로가 함께 쓰는데, 그중 하나(덱)의
      // 매니페스트가 표지를 들고 있으면 나머지 둘이 덱을 거쳐야만 표지를 얻는다.
      // 여기서 정하는 것은 **어느 자리에 놓을지**(순서 · 섹션 라벨)뿐이다.
      // 목차가 쓰던 제목 · 기간 · 배지는 표지의 `toc` 가 내놓는다 (engine.js).
      { layout: 'projectCover', section: 'Main · Engine', slug: 'dx11-engine' },
      ...
      diagram('01 엔진 — 구조', '소유권으로 나눈 엔진 아키텍처',
        D.overview.architecture, 'DXArchitectureViz'),

      diagram('01 엔진 — 흐름', '한 프레임의 실행 순서',
      ...
      diagram('02 물리 — 구조', '물리 상태 소유권과 네 통신 경로',
        D.physics.boundary, 'DXBoundaryViz', {
          lead: '시뮬레이션 상태를 게임 객체에서 `FPhysicsStateArrays`의 23개 속성 배열로 옮기고, 게임과 물리 사이의 데이터 이동을 네 통로로 제한했다.',
        }),

      diagram('02 물리 — 흐름', '물리 틱의 게이트와 격리 구간',
        D.physics.tick, 'DXTickViz'),

      codeSlide('02 물리 — 흐름 · CODE', '시뮬레이션 준비·마무리 경계', D.code.tick),

      diagram('02 물리 — 입력 동기화', '갱신 빈도별 물리 입력 3계층',
        D.physics.sync, 'DXSyncTierViz'),

      codeSlide('02 물리 — 입력 동기화 · CODE', '더티 플래그 기반 계층별 입력 복사', D.code.sync),
      ...
      codeSlide('03 렌더링 — 세부 · CODE', '렌더 데이터 기반 드로우 호출 조립', D.code.renderCache),
    ],
  };
})();
