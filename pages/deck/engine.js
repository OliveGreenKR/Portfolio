...
//   Cartapli: Fold Quest 절(6장) · Wobble Wobble · 교육용 게이미피케이션(외주) · Labs 5종
...
(function buildEngineDeck() {
  ...
  const FULL = ['intro', 'cm', 'motelet', 'dx11', 'outro'];
  ...
  if (spans.length && spans[0].key === 'intro') {
    ...
    const entries = spans
      ...
      .map((sp, i) => {
        const slide0 = sp.part.slides[0] || {};
        // 표지를 프로젝트가 소유하면(layout:'projectCover') 목차 재료도 표지가 내놓는다.
        // 아직 옮기지 않은 프로젝트는 예전처럼 매니페스트 슬라이드에서 읽는다.
        const own = ((window.COVERS || {})[slide0.slug] || {}).toc;
        const cover = own
          ? { title: own.title, period: own.period, pills: (own.tags || []).map((t) => ({ text: t })) }
          : slide0;
        return {
          ...
          // 기간은 태그가 아니라 제목 옆 한 칸이다 — 세 줄의 같은 자리에 있어야 훑어서 비교된다.
          // 값은 각 프로젝트 data.js 의 meta.period 를 매니페스트가 표지에 얹은 것이다.
          period: cover.period || null,
          // 기간이 제목 옆으로 올라갔으니 배지에서는 뺀다 — dx11 표지 pill 이
          // '2025.01 – 2025.08 · 1인' 처럼 기간을 물고 있어 그대로 두면 한 줄에 두 번 나온다.
          // 배지를 통째로 버리지 않는다: 기간만 떼고 남는 말('1인')은 그대로 태그로 쓴다.
          tags: (cover.pills || [])
            .map((p) => tag(p.text))
            .map((t) => (cover.period ? t.replace(cover.period, '').replace(/^\s*[·—-]\s*/, '').trim() : t))
            .filter(Boolean),
          from: page(sp.start),
          to: page(sp.end),
