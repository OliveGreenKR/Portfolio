...
(function defineCMSlides() {
  ...
  function CMDeckFlowDiagram({ lanes, clock }) {
    ...
    const rows = [
      { y: 46, tag: 'VARIABLE FRAME', note: lanes[0].note, items: lanes[0].items, tone: 'v' },
      { y: 152, tag: 'FIXED STEP', note: lanes[1].note, items: lanes[1].items, tone: 'f' },
      { y: 292, tag: 'PRESENTATION', note: lanes[2].note, items: lanes[2].items, tone: 'p' },
    ];
    const H = 62, GAP = 13;
    ...
    const brace = `M${L} 224 v10 h${R - L} v-10`;
    ...
    return (
      <figure className="cmd-flowchart">
        <svg viewBox={`0 0 ${W} 372`} role="img"
          ...
          <text x={(L + R) / 2} y="254" textAnchor="middle" className="cmd-fl-repeat">{clock[2]}</text>
  ...
  function CMDFlow({ s }) {
    ...
    return (
      <div className="sl-body cmd-flow">
        ...
        {/* dl 을 3열 grid 로 두면 dt·dd 가 짝을 잃고 칸을 가로질러 흩어진다. 쌍마다 한 칸. */}
        <div className="cmd-rules">
          {a.decisions.map(([title, body]) => (
            <div key={title}><b>{title}</b><span>{RI(body)}</span></div>
          ))}
        </div>
      </div>
    );
